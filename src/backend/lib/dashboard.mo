...
module {
  ...
  public func seedMockData(state : State) : Text {
    ...
    let observations : [(Text, ObsTypes.ObservationRecord)] = [
      ("obs-1", { id = "obs-1"; obsType = #unsafeAct; description = "Worker observed bypassing lockout procedure on press machine"; location = "Forge Shop"; reportedBy = "Rajesh Kumar"; status = #closed; actions = ["Immediate stop-work issued", "Toolbox talk conducted", "Disciplinary action recorded"]; createdAt = 1_699_800_000_000_000_000 }),
      ("obs-2", { id = "obs-2"; obsType = #unsafeCondition; description = "Oil leak on floor near HTL-5 creating slip hazard"; location = "HTL Area"; reportedBy = "Priya Patel"; status = #inProgress; actions = ["Warning cones placed", "Maintenance work order raised"]; createdAt = 1_700_200_000_000_000_000 }),
      ("obs-3", { id = "obs-3"; obsType = #nearMiss; description = "Rolling bloom almost struck maintenance worker in aisle – Rolling Mill"; location = "Rolling Mill"; reportedBy = "Arjun Singh"; status = #closed; actions = ["Aisle barrier installed", "Coordination procedure updated"]; createdAt = 1_700_500_000_000_000_000 }),
      ("obs-4", { id = "obs-4"; obsType = #nearMiss; description = "Crane hook swung unexpectedly during load positioning"; location = "Forge Shop"; reportedBy = "Sunil Sharma"; status = #open; actions = ["Load secured and operation halted"]; createdAt = 1_700_800_000_000_000_000 }),
      ("obs-5", { id = "obs-5"; obsType = #unsafeCondition; description = "Fire extinguisher missing from marked location at RHF Furnace"; location = "RHF Furnace"; reportedBy = "Rajesh Kumar"; status = #closed; actions = ["Replacement extinguisher installed same day", "Monthly check schedule updated"]; createdAt = 1_701_000_000_000_000_000 }),
      ("obs-6", { id = "obs-6"; obsType = #unsafeAct; description = "Contractor personnel working in confined space without gas test"; location = "Utility Area"; reportedBy = "Mehul Contractor"; status = #closed; actions = ["Work stopped immediately", "Gas test conducted", "Contractor briefed on PTW requirements"]; createdAt = 1_701_300_000_000_000_000 }),
      ("obs-7", { id = "obs-7"; obsType = #nearMiss; description = "Electrical panel door left open near water spray area"; location = "RHF Furnace"; reportedBy = "Sunil Sharma"; status = #inProgress; actions = ["Panel secured", "Fault reported to electrical team"]; createdAt = 1_701_600_000_000_000_000 }),
      ("obs-8", { id = "obs-8"; obsType = #unsafeCondition; description = "Emergency exit sign not illuminated at Maintenance Workshop"; location = "Maintenance Workshop"; reportedBy = "Arjun Singh"; status = #open; actions = ["Electrician notified for replacement"]; createdAt = 1_701_900_000_000_000_000 }),
    ];
